...
namespace BankApp.Application.Transactions.Queries.GetAccountTransactions
{
    public class GetAccountTransactionsQueryHandler : IRequestHandler<GetAccountTransactionsQuery, List<Transaction>>
    {
        ...
        {
            var model = await _context.Transactions.AsNoTracking().Where(t => t.AccountId == request.AccountId).OrderByDescending(a => a.Date).Skip(20 * request.Page).Take(20).ToListAsync();
            return model;
        }
    }
}
